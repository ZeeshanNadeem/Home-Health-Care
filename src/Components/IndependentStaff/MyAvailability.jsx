...
import MultiServices from "../ReactMultiSelect/MultiServicesIndep";
...
const MyAvailability = () => {
  ...
  useEffect(async () => {
    ...
    try {
      ...
      for (let s of services.results) {
        totalService_.push({ label: s.serviceName, value: s.serviceName })
      }
      setTotalServices(totalService_)
      console.log("staff Detials::", StaffDetails.staffSpeciality)
      let multiple_services = [];
      for (let s of StaffDetails.staffSpeciality) {
        multiple_services.push({ label: s.serviceName, value: s.serviceName })
      }
      setMultipleServices(multiple_services)
  ...
  const updateDetials = async (e) => {
    ...
    if (!errors) {
      const servicesMulti = JSON.parse(localStorage.getItem("servicesMultiIndep"));


      let myServices = [];
      for (let s of indepServices.results) {
          ...
          >
            ...
            <Row>
              ...
              <Col>
                ...
                <Form.Group className="mb-3" >
                  <Form.Label className="label-order label-style1">Services</Form.Label>
                  {/* <Form.Select
                    onChange={onChangeTextFields}
                    name="service"
                    ...
                    aria-label="Default select example"> */}
                  {multipleServices != undefined &&
                    multipleServices.length > 0 &&
                    <MultiServices
                      services={totalServices}
                      PreSelectedService={multipleServices}
                    />}
