...
const PatientMap = () => {
  ...
  const onMapClick = React.useCallback(async (event) => {
    ...
    if (
      ...
      await axios.put(
        configApi.apiEndPoint + `/user/${user._id}?updatePatientLocation=true`,
        {
  ...
  const handleRadius = (e) => {
    ...
    } else markers[markers.length - 1].radius = e.currentTarget.value;
  ...
  const setSelectedMarker = (marker) => {
    markers = markers.map((m) => {
      if (m.selected === true) delete m.selected;
  ...
  const onMarkerDrag = async (e, marker) => {
          ...
          >
            {showAlert && (
              <div className="alert-container">
                <div
                  className="alert alert-success cutomization-alert"
                  role="alert"
                >
                  ...
                  icon={{
                    url:
                      ...
                      (index === markers.length - 1 &&
                        !markers.some((x) => x.selected === true))
                        ? "/pinBlue.png"
                        : "/pin.png",
...
function Search({ panTo }) {
  ...
  return (
    ...
    >
      ...
      <ComboboxPopover className="popover-style">
        <ComboboxList>
          {status === "OK" &&
