...
import EditModal from "../Modals/MoodleForEdit";
import BasicModal from "../Modals/AddServiceModle";
...
const EditService = (props) => {
  ...
  const getTotalDocuments = async () => {
    ...
    const { data: totalDocuments } = await axios.get(
      `http://localhost:3000/api/independentServices`
    );
  ...
  const PopulateTable = async () => {
    ...
    const { data } = await axios.get(
        `http://localhost:3000/api/independentServices?page=${pageSelected}&limit=${pageSize}`
    );
  ...
  const deleteService = async (id) => {
    ...
    try {
      await axios.delete("http://localhost:3000/api/independentServices/" + id);
      // toast.success("Deleted");

      await axios.get("http://localhost:3000/api/independentServices");
      const { data: servicesPerCurrentPage } = await axios.get(
        `http://localhost:3000/api/independentServices?page=${pageSelected}&limit=${pageSize}`
      );
      if (servicesPerCurrentPage.results.length === 0)
        window.location = "/app/admin/services";
    } catch (ex) {
  ...
  const updateService = async () => {
    ...
    const { data: allOrganizations } = await axios.get(
      `http://localhost:3000/api/independentServices?page=${totalPages}&limit=${pageSize}`
    );
                  ...
                  <tbody>
                    {services.map((data) => (
                      <tr key={data._id}>
                        ...
                        <td>{data.serviceOrganization && data.serviceOrganization.name}</td>
